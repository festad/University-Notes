...
\begin{frame}{Exploring the solutions}
    ...
    in a range between 5 and 14 to contain the duration of the experiments.
\end{frame}

\begin{frame}{Solutions found}
    It is intuitive that the number of solutions found increases
    with the number of sets and it decrease with the cardinality of the domain.
...
\begin{frame}{Explored nodes}
    ...
    as the graphs in figure (\ref{fig:base_star_5x5}), which describe the time required to find all
    the solutions to the same problems.
    Why, fixing the number of rows, a smaller number of columns implies a greater
...
\begin{frame}{}
    \begin{figure}
        ...
        is called to examine a portion of the matrix A, then it needs to 
        read other portions of the matrix A which are not passed to it from outside.}
    \end{figure}
\end{frame}
