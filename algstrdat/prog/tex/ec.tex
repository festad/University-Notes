...
\begin{frame}{Loading portions}
    ...
    when the dimension is small, then the advantage seem to decrease.
    In order to understand whether decreasing the dimension of the chunk
    of loaded rows is always advantageous, we would need to test on much
    larger instances, and unfortunately the time required to solve such
    instances is too long. 
\end{frame}

\begin{frame}
    % side by side figures "load_portions_a" and "load_portions_b"
    \begin{figure}
        \centering
        \includegraphics[width=0.45\textwidth]{loadable_rows_time_diff.pdf}
        % \caption{}
        \label{fig:load_portions_a}
    \end{figure}
    \begin{figure}
        \centering
        \includegraphics[width=0.45\textwidth]{loadable_rows_time_diff_rev.pdf}
        % \caption{}
