...
\begin{frame}{}
    Figure (\ref{fig:mem_array}) shows different data structures to store a list of boolean values.
    The first two scenarios show that the memory required to a list of
    ...
    No advantage comes from using \texttt{boolean} instead of \texttt{integer}.
    ...
    to be considered as memory occupation, they are not actually used to store
    the values, they are used only when the value is accessed.
    ...
    is accessed, the Python runtime environment again creates the object on the fly.
\end{frame}
...
\begin{frame}{}
        ...
        \begin{figure}
            ...
            \caption{Notice the difference amond the two cases. In the first scenario we 
